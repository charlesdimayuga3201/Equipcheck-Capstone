...
function UpdateS(props) {
  ...
  useEffect(() => {
    ...
    const fetchBuildingOptions = async () => {
      const Buildings = [];
      const q = collection(firebase, "ListSprinkler");
  ...
  useEffect(() => {
    ...
    const fetchFloorOptions = async () => {
      ...
      const Floors = [];
      const q = query(
        collection(firebase, "ListSprinkler"),
        orderBy("floor", "asc"),
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const docData = {
        ...
        date: currentDate,
        time: currentTime,
        sprinklerhead: switchValues.switch1 ? "check" : "notworking",
        piping: switchValues.switch2 ? "check" : "notworking",
        valves: switchValues.switch3 ? "check" : "notworking",
        pressureregulator: switchValues.switch4 ? "check" : "notworking",
        controlpanel: switchValues.switch5 ? "check" : "notworking",
        backflowpreventer: switchValues.switch6 ? "check" : "notworking",
            ...
            {selectedSafetyEquipment ? (
              <>
                <View style={styles.column1}>
                  <Text style={styles.datafont1}>{currentDate}</Text>
                </View>
                <View style={styles.column1}>
                  <Text style={styles.datafont1}>{currentTime}</Text>
                </View>
                <View style={styles.column1}>
                  <Switch
