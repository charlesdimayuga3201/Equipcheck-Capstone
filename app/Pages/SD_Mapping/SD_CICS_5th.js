...
export default function SD_CICS_5th({
  isModalVisible,
  hideModal,
  selectedIcon,
  showModal,
  ...
  const [SD54, setSD54] = useState([]);
  useEffect(() => {
    const SD54Data = [];
    const SD54Collections = collection(firebase, "SD54");
    const fetchSD54Data = async () => {
      try {
        const q54 = query(
          SD54Collections,
          orderBy("date", "desc"),
        ...
        querySnapshot54.forEach((doc) => {
          ...
          SD54Data.push(data54);
          // Use the data as needed
          console.log("Fetched FE54 data:", data54);
        });
        console.log(SD54Data);
        setSD54(SD54Data);
      } catch (error) {
        console.error("Error fetching FE54:", error);
      }
    };
    fetchSD54Data();
  }, []);
  const [SD55, setSD55] = useState([]);
  useEffect(() => {
    const SD55Data = [];
    const SD55Collections = collection(firebase, "SD55");
    const fetchSD55Data = async () => {
      try {
        const q55 = query(
          SD55Collections,
          orderBy("date", "desc"),
        ...
        querySnapshot55.forEach((doc) => {
          ...
          SD55Data.push(data55);
          // Use the data as needed
          console.log("Fetched FE55 data:", data55);
        });
        console.log(SD55Data);
        setSD55(SD55Data);
      } catch (error) {
        console.error("Error fetching FE55:", error);
      }
    };
    fetchSD55Data();
  }, []);
  const [SD56, setSD56] = useState([]);
  useEffect(() => {
    const SD56Data = [];
    const SD56Collections = collection(firebase, "SD56");
    const fetchSD56Data = async () => {
      try {
        const q56 = query(
          SD56Collections,
          orderBy("date", "desc"),
        ...
        querySnapshot56.forEach((doc) => {
          ...
          SD56Data.push(data56);
          // Use the data as needed
          console.log("Fetched FE56 data:", data56);
        });
        console.log(SD56Data);
        setSD56(SD56Data);
      } catch (error) {
        console.error("Error fetching FE56:", error);
      }
    };
    fetchSD56Data();
  }, []);
  const [SD57, setSD57] = useState([]);
  useEffect(() => {
    const SD57Data = [];
    const SD57Collections = collection(firebase, "SD57");
    const fetchSD57Data = async () => {
      try {
        const q57 = query(
          SD57Collections,
          orderBy("date", "desc"),
        ...
        querySnapshot57.forEach((doc) => {
          ...
          SD57Data.push(data57);
          // Use the data as needed
          console.log("Fetched FE57 data:", data57);
        });
        console.log(SD57Data);
        setSD57(SD57Data);
      } catch (error) {
        console.error("Error fetching FE57:", error);
      }
    };
    fetchSD57Data();
  }, []);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView horizontal>
        <ImageBackground
          source={require("../../assets/images/SD_CICS_5th.png")}
          resizeMode="contain"
          style={styles.image}
