...
export default function FE_SSC_3rd({
  isModalVisible,
  hideModal,
  selectedIcon,
  showModal,
  ...
  const [FE158, setFE158] = useState([]);
  useEffect(() => {
    const Fe158Data = [];
    const FE158Collections = collection(firebase, "FE158");
    const fetchFe158Data = async () => {
      try {
        const q158 = query(
          FE158Collections,
          orderBy("date", "desc"),
  ...
  const [FE159, setFE159] = useState([]);
  useEffect(() => {
    const Fe159Data = [];
    const FE159Collections = collection(firebase, "FE159");
    const fetchFe159Data = async () => {
      try {
        const q159 = query(
          FE159Collections,
          orderBy("date", "desc"),
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView horizontal>
        <ImageBackground
          source={require("../../assets/images/FESSC_3rd.png")}
          resizeMode="contain"
          style={styles.image}
          imageStyle={styles.image_imageStyle}
        >
          <View style={styles.fE1Row}>
            {FE158.map((item, index) => (
              <TouchableOpacity
                key={index}
                style={styles.fE1}
