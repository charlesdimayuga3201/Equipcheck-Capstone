...
export default function ViewAllFe() {
  ...
  useEffect(() => {
    const fetchLatestDocuments = async () => {
      ...
      for (let i = 1; i <= 71; i++) {
        const feCollection = collection(firebase, `FE${i}`);
  ...
  //   const countConditionTypes = (condition) => {
  //     return latestDocuments.filter((doc) => doc.condition === condition).length;
  //   };

  //   const countTotalGoodCondition = () => {
  //     return latestDocuments.filter((doc) => doc.condition === "good").length;
  //   };
  return (
        ...
        <View style={styles.rect}>
          ...
          <View style={styles.containerA}>
            <View style={styles.column}>
              <Text style={styles.datafont}>Fire Exinguisher</Text>
            </View>
